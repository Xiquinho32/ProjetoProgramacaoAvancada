...
import pt.ipg.projeto1.databinding.FragmentListaDoentesBinding
import pt.ipg.projeto1.databinding.FragmentListaMedicosBinding

class ListaDoentesFragment : Fragment(), LoaderManager.LoaderCallbacks<Cursor>{
    var doenteSeleccionado : Doentes? = null
        get() = field
        set(value) {
            field = value
            (requireActivity() as MainActivity).mostraOpcoesAlterarEliminar(field != null)
        }
    private var _binding: FragmentListaDoentesBinding? = null
    private var adapterDoentes: AdapterDoentes? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentListaDoentesBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapterDoentes = AdapterDoentes(this)
        binding.recyclerViewDoentes.adapter = adapterDoentes
        binding.recyclerViewDoentes.layoutManager = LinearLayoutManager(requireContext())
    ...
    fun processaOpcaoMenu(item: MenuItem) : Boolean =
        when(item.itemId) {
            R.id.action_inserir -> {
                ...
                (activity as MainActivity).atualizaData(getString(R.string.inserir_doentes_label))
                true
            }
            R.id.action_alterar -> {
                val acao = ListaDoentesFragmentDirections.actionListaDoentesFragmentToEditarDoentesFragment(doenteSeleccionado)
                findNavController().navigate(acao)
                (activity as MainActivity).atualizaData(getString(R.string.alterar_doentes_label))
                true
            }
