...
class ListaEspecialidadesFragment : Fragment(), LoaderManager.LoaderCallbacks<Cursor> {
    var especialidadeSeleccionado: Especialidades? = null
        get() = field
        set(value) {
            field = value
    ...
    fun processaOpcaoMenu(item: MenuItem) : Boolean =
        when(item.itemId){
            ...
            R.id.action_eliminar -> {
                val acao = ListaEspecialidadesFragmentDirections.actionListaEspecialidadesFragmentToEliminarEspecialidadesFragment(especialidadeSeleccionado!!)
                findNavController().navigate(acao)
                true
            }
            else -> false
